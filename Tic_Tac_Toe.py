...
def check_winner():
    ...
    for row in range(3):
        if (board[row][0]["text"] == board[row][1]["text"] == board[row][2]["text"]
            ...
            for column in range(3):
                board[row][column].config(foreground=color_blue, background=color_light_gray)
            game_over = True
            return
        
    #vertically, check 3 column
    for column in range(3):
    ...
    if (board[0][0]["text"] == board[1][1]["text"] == board[2][2]["text"]
        ...
        for i in range(3):
            board[i][i].config(foreground=color_blue, background=color_light_gray)
        game_over = True
        return
    ...
    if (board[0][2]["text"] == board[1][1]["text"] == board[2][0]["text"]
        ...
        board[0][2].config(foreground=color_blue, background=color_light_gray)
        board[1][1].config(foreground=color_blue, background=color_light_gray)
        board[2][0].config(foreground=color_blue, background=color_light_gray)
        game_over = True
        return
...
def new_game():
    ...
    for row in range(3):
        for column in range(3):
            board[row][column].config(text="", foreground=color_peach, background=color_gray)
...
for row in range(3):
    for column in range(3):
        board[row][column] = tkinter.Button(frame, text="", font=("Consolas", 50, "bold"),
                                            background=color_gray, foreground=color_peach, width=4, height=1,
